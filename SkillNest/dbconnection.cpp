...
bool DBConnection::createAnnouncement(int courseId, string courseTitle, string message, string postedBy) {
    ...
    try {
        ...
        cout << "Announcement saved successfully!" << endl;
        return true;
    }

    catch (sql::SQLException& e) {
        cout << "Announcement creation failed!" << endl;
        cout << "SQL ERROR: " << e.what() << endl;
        return false;
    }
}
...
void DBConnection::addNotification(string email, string message) {
    ...
    try {
        ...
        cout << "Notification added for: " << email << endl;
    }
    catch (sql::SQLException& e) {
        cout << "DB ERROR (Notification): " << e.what() << endl;
    }
}
...
bool DBConnection::courseExists(int courseId) {
    ...
    try {
        ...
        cout << "Course validation failed: " << e.what() << endl;
        return false;
    }
}

bool DBConnection::isCourseApproved(int courseId) {

    try {
        sql::PreparedStatement* pstmt = con->prepareStatement(
            "SELECT approved FROM courses WHERE id = ?"
        );
        ...
        if (res->next()) {
            approved = res->getInt("approved") == 1;
        }
        ...
        cout << "Course approval check failed: " << e.what() << endl;
        return false;
    }
}

bool DBConnection::isAlreadyEnrolled(string studentEmail, int courseId) {

    try {
        sql::PreparedStatement* pstmt = con->prepareStatement(
            "SELECT id FROM enrollments WHERE student_email = ? AND course_id = ?"
        );
        ...
        cout << "Enrollment check failed: " << e.what() << endl;
        return false;
    }
}

bool DBConnection::removeEnrollment(string studentEmail, int courseId) {

    try {
        sql::PreparedStatement* pstmt = con->prepareStatement(
            "DELETE FROM enrollments WHERE student_email = ? AND course_id = ?"
        );
        ...
        cout << "Remove enrollment failed: " << e.what() << endl;
        return false;
    }
}

void DBConnection::removeUnapprovedEnrollmentsForStudent(string studentEmail) {
    ...
    try {
        ...
        cout << "Unapproved enrollments cleaned for: " << studentEmail << endl;
    }

    catch (sql::SQLException& e) {
        cout << "Cleaning unapproved enrollments failed: " << e.what() << endl;
    }
}