...
html += c->getFeatures();
html += "<button type='submit'>Save Features</button>";
html += "</form>";
html += "</td>";
            html += "</tr>";
...
bool DBConnection::saveStudentFeatures(string email, string courseTitle, bool cert, bool notes, bool videos) {
    try {
        ...
        string checkQuery =
            "SELECT id FROM student_features WHERE student_email='" + email +
        ...
        if (res->next()) {
            query =
                "UPDATE student_features SET certificate=" + to_string(cert) +
            ...
            query =
                "INSERT INTO student_features (student_email, course_title, certificate, notes, videos) VALUES ('" +
                email + "', '" + courseTitle + "', " +
                to_string(cert) + ", " +
                to_string(notes) + ", " +
                to_string(videos) + ")";
        ...
        cout << "Saving features failed: " << e.what() << endl;
        return false;
    }
}
vector<bool> DBConnection::getSavedFeatures(string email, string courseTitle) {
    ...
    try {
        ...
        string query =
            "SELECT certificate, notes, videos FROM student_features "
            "WHERE student_email='" + email +
            "' AND course_title='" + courseTitle + "'";
        ...
        if (res->next()) {
            features[0] = res->getBoolean("certificate");
            features[1] = res->getBoolean("notes");
            features[2] = res->getBoolean("videos");
        }
        ...
        cout << "Feature load failed: " << e.what() << endl;
    }
